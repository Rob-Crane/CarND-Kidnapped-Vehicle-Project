...
int main() {
  ...
  h.onMessage([&pf, &map, &delta_t, &sensor_range, &num_particles, &sigma_pos,
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (!pf.initialized()) {
            ...
            pf.init(num_particles, sense_x, sense_y, sense_theta, sigma_pos,
                    sigma_landmark, sensor_range, map);
          } else {
          ...
          for (unsigned int i = 0; i < x_sense.size(); ++i) {
            LandmarkObs obs(x_sense[i], y_sense[i]);
          ...
          std::vector<Particle>::const_iterator best_particle;
          double weight_sum = 0.0;
          for (std::vector<Particle>::const_iterator particle_it =
                   particles.cbegin();
               particle_it != particles.cend(); ++particle_it) {

            if (particle_it->weight() > highest_weight) {
              highest_weight = particle_it->weight();
              best_particle = particle_it;
            }
